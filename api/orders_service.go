...
func PlaceOrder(p Param) (interface{}, error) {
	...
	if valid := nova.IsValidOrderSignature(order.Address, order.ID, order.Signature); !valid {
...
func checkBalanceAllowancePriceAndAmount(order *BuildOrderReq, address string) error {
	...
	baseTokenBalance := nova.GetTokenBalance(market.BaseTokenAddress, address)
	baseTokenAllowance := nova.GetTokenAllowance(market.BaseTokenAddress, os.Getenv("NSK_PROXY_ADDRESS"), address)
	...
	quoteTokenBalance := nova.GetTokenBalance(market.QuoteTokenAddress, address)
	quoteTokenAllowance := nova.GetTokenAllowance(market.QuoteTokenAddress, os.Getenv("NSK_PROXY_ADDRESS"), address)
...
func BuildAndCacheOrder(address string, order *BuildOrderReq) (*BuildOrderResp, error) {
	...
	orderData := nova.GenerateOrderData(
		int64(2),
	...
	orderJson := models.OrderJSON{
		Trader:                  address,
		Relayer:                 os.Getenv("NSK_RELAYER_ADDRESS"),
	...
	orderHash := nova.GetOrderHash(sdkOrder)
