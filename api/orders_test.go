...
func mockMarketDao() {
	...
	marketWethDai := &models.Market{
		...
		BaseTokenAddress:   os.Getenv("NSK_WETH_TOKEN_ADDRESS"),
		BaseTokenDecimals:  18,
		QuoteTokenSymbol:   "DAI",
		QuoteTokenAddress:  os.Getenv("NSK_USD_TOKEN_ADDRESS"),
...
func mockErc20() {
	mockNova := sdk.MockNova{
		&ethereum.EthereumNovaProtocol{},
		&sdk.MockBlockchain{},
	}
	...
	mockNova.BlockChain.(*sdk.MockBlockchain).On("GetTokenBalance", mock.Anything, mock.Anything).Times(10).Return(aBigNumber, nil)
	mockNova.BlockChain.(*sdk.MockBlockchain).On("GetTokenAllowance", mock.Anything, mock.Anything, mock.Anything).Times(10).Return(aBigNumber, nil)
	mockNova.BlockChain.(*sdk.MockBlockchain).On("GetHotFeeDiscount", mock.Anything).Times(10).Return(decimal.New(1, 2))
	mockNova.BlockChain.(*sdk.MockBlockchain).On("IsValidSignature", mock.Anything, mock.Anything, mock.Anything).Return(true, nil)
}
...
func (suite *BuildOrderTestSuit) request(body interface{}) *Response {
	...
	req.Header.Set("Nova-Authentication", address+"#"+message+"#"+signatrue)
