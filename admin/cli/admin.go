...
type IAdminApi interface {
	...
	BalanceOf(tokenAddress, address string) ([]byte, error)
...
func (a *Admin) BalanceOf(tokenAddress, address string) (ret []byte, err error) {
	var amount *big.Int
	err, amount = a.erc20.BalanceOf(tokenAddress, address)

	ret = []byte(amount.String())
