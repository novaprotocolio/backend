...
func NewDexCli() *cli.App {
	...
	app := cli.NewApp()
	app.Usage = "A tool to manage nova dex"
	app.Version = "0.0.1"
	app.Name = "nova-dex-ctl"
    ...
    nova-dex-ctl market approve HOT-WWW`,
					Action: func(c *cli.Context) error {
						marketID = c.Args().Get(0)
						if len(marketID) == 0 {
							return cli.ShowSubcommandHelp(c)
    ...
    nova-dex-ctl market unpublish HOT-WWW`,
					Action: func(c *cli.Context) error {
						marketID = c.Args().Get(0)
						if len(marketID) == 0 {
							return cli.ShowSubcommandHelp(c)
    ...
    nova-dex-ctl market changeFees HOT-WETH "0.001" "0.003"`,
			...
			Subcommands: cli.Commands{
				{
					...
					Action: func(c *cli.Context) error {
						...
						if len(address) == 0 {
							fmt.Println("missing arguments, usage: nova-dex-cli address orders address --marketID=xxx")
							return nil
						}
				...
				},
				{
					...
					Action: func(c *cli.Context) error {
						address := c.Args().Get(0)
						if len(address) == 0 {
							fmt.Println("missing arguments, usage: nova-dex-cli address balances address")
							return nil
						}
				...
				},
				{
					...
					Action: func(c *cli.Context) error {
						address := c.Args().Get(0)
						if len(address) == 0 {
							fmt.Println("missing arguments, usage: nova-dex-cli address trades address --marketID=xxx")
							return nil
						}
			...
			Subcommands: cli.Commands{
				{
					...
					Action: func(c *cli.Context) error {
						...
						if len(orderID) == 0 {
							fmt.Println("missing arguments, usage: nova-dex-cli order cancel orderID")
							return nil
						}
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				address := c.Args().Get(0)
				if len(address) == 0 {
					fmt.Println("missing arguments, usage: nova-dex-cli balanceOf address --tokenAddress=")
					return nil
				}

				printIfErr(admin.BalanceOf(tokenAddress, address))
