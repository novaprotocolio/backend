...
func (m MarketHandler) handleNewOrder(event *common.NewOrderEvent) (transaction *models.Transaction, launchLog *models.LaunchLog) {
	...
	// we should push this message to orderbook protocol and get back the matchResult instead of
	// single orderbook in memory
	order := map[string]string{
		"pair_name": eventOrder.MarketID,
		"type" : eventOrder.Type,
		"side": "ask",
		"quantity":eventOrder.Amount.String(),
		"price" : eventOrder.Price.String(),
		"trade_id" : eventOrder.TraderAddress,
		"order_id" : "0",
	}

	if eventOrder.Side == "buy" {
		order["side"] = "bid"
	}

	bytes := novaProtocol.ProcessOrderbook(order)

	utils.Debugf("new order result :%s", string(bytes))
	...
	matchResult, hasMatch := m.novaEngine.HandleNewOrder(eventMemoryOrder)
	if hasMatch {
...
func processTransactionAndLaunchLog(matchResult *MatchResultWithOrders) (*models.Transaction, *models.LaunchLog) {
	...
	novaTakerOrder := getNovaOrderFromModelOrder(takerOrder.GetOrderJson())

	var novaMakerOrders []*sdk.Order
	...
	for _, item := range matchResult.MatchItems {
		...
		novaMakerOrder := getNovaOrderFromModelOrder(modelMakerOrder.GetOrderJson())
		novaMakerOrders = append(novaMakerOrders, novaMakerOrder)
	...
	launchLog := &models.LaunchLog{
		...
		Status:    "created",
		From:      os.Getenv("NSK_RELAYER_ADDRESS"),
		To:        os.Getenv("NSK_HYBRID_EXCHANGE_ADDRESS"),
		Value:     decimal.Zero,
		GasLimit:  int64(len(matchResult.MatchItems) * 250000),
		Data:      utils.Bytes2HexP(novaProtocol.GetMatchOrderCallData(novaTakerOrder, novaMakerOrders, baseTokenFilledAmounts)),
		CreatedAt: time.Now().UTC(),
...
func (m *MarketHandler) handleCancelOrder(event *common.CancelOrderEvent) (interface{}, error) {
	...
	msg, success := m.novaEngine.HandleCancelOrder(bookOrder)
	if success {
		pushMessage(msg)
	}
...
func NewMarketHandler(ctx context.Context, market *models.Market, engine *engine.Engine) (*MarketHandler, error) {
	...
	// re-insert available orders into NovaEngine
